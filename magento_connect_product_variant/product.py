...
class product_product(osv.osv):
    ...
    def magento_create_product_configurable(self, cr, uid, magento_app, product, store_view, context = None):
        ...
        if not external_referential_id: #create
            ...
            product_template_id = self.pool.get('product.template').create(cr, uid, values, context)
            self.pool.get('magento.external.referential').create_external_referential(cr, uid, magento_app, 'product.template', product_template_id, product_template_magento_id)
            LOGGER.notifyChannel('Magento Sync API', netsvc.LOG_INFO, "Create Product Template: magento app %s, openerp id %s, magento product id %s." % (magento_app.name, product_template_id, product['product_id']))
            ...
            product_template_obj = self.pool.get('product.template').browse(cr, uid, product_template_id, context)
            ...
            try:
                ...
                if len(dimesion_types)>0:
                    self.pool.get('product.template').add_all_option(cr, uid, [product_template_id], context)
            except:
                LOGGER.notifyChannel('Magento Sync API', netsvc.LOG_ERROR, "Write Product Template: magento %s, openerp id %s, magento product id %s." % (magento_app.name, product_template_id, product['product_id']))
                self.pool.get('magento.log').create_log(cr, uid, magento_app, 'product.template', product_template_id, product['product_id'], 'error', _('Check variants from Magento. Need add dimensions at OpenERP manually') )
                pass
