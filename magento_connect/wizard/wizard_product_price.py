...
class magento_sync_price_wizard(osv.osv_memory):
    _name = 'magento.sync.price.wizard'
    ...
    def sync_price(self, cr, uid, ids, data, context={}):
        ...
        if len(product_ids) > 0:
            thread1 = threading.Thread(target=self.pool.get('sale.shop').magento_export_prices_stepbystep, args=(cr.dbname, uid, magento_app.id, shop.id, [prod.id], context))
            thread1.start()
        return True

magento_sync_price_wizard()
